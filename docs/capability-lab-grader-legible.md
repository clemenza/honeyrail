...
Resolving the image is not the same as establishing isolation, and the two are checked separately. An image can exist and `docker run` still fail before any container is created — an undefined network, a daemon fault — and on disk that failure is indistinguishable from an agent that ran and submitted nothing. So each attempt proves its own container: the container touches a marker file in the bind-mounted facade channel before the agent process starts, and the host reads it back afterwards as `telemetry.isolationEstablished`. An attempt whose container never started is attributed `isolation_or_integrity` / `infrastructure_error`, never `agent_invalid_submission`.
