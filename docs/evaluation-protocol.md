...
| Capability Lab: grader-legible reproducer (#237) | Six synthetic TRAIN archetypes, external raw-channel grader, failure-stage attribution and `primaryCause` census, paired baseline/candidate runner, `D/A` primary endpoint, opt-in Docker isolation and declared agent identity gating eligibility, fail-closed artifact roots, content-hashed frozen candidate | Harness-validated with scripted shapes only; a registered real-agent pilot is required before any capability claim, and transfer belongs on the reserved unseen family. Isolation is opt-in and Docker-only: an unisolated run is ineligible, not merely caveated |
