...
- `dsh` (#87/#88) is **experimental**: DSH (DeepSeek Harness), a developer-preview CLI expected to break between releases (pin an exact version, e.g. `@deepseek-ai/dsh@0.1.0-rc.7`; requires Node >= 24 — see [docs/dsh-adapter-notes.md](dsh-adapter-notes.md) for the full spike). Launches `dsh --profile headless --patch cordis.patch.yml <prompt>`, followed by a done marker and an idle-forever tail (`; echo HR_DSH_DONE; while :; do sleep 3600; done`) since headless is a real one-shot subprocess that exits on its own rather than an idling TUI. Assumes a `cordis.patch.yml` already exists at the worktree root, written by the `instructionFile` mechanism before launch (baseline injects a no-op `[]` patch; a candidate variant patches the composed profile's `system-prompt` plugin's `config.persona` field — the instruction-layer route the #87 spike confirmed both statically via `--dump-config` and behaviorally via a live run). Requires `DEEPSEEK_API_KEY` in the launch environment; headless has no interactive-prompt answerer at all (`interactivePrompts: false` is a structural guarantee, not just unimplemented), and any tool call needing escalation beyond the sandbox fails closed instead of hanging. `modelSelection` is `false` — model choice is patch-only (`agent-default-model`'s `config.model`) with no CLI flag, and this adapter doesn't yet write a second patch file to route it. `findFatalError` covers the two failure shapes seen in the spike: a missing-credential error and a plugin/tree boot failure (e.g. from a Node-version mismatch).
