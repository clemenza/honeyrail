...
`withPostgresResearchEnvironment()`'s `body` receives an `AbortSignal` alongside `env`. When the outer `timeoutMs` elapses, that signal is aborted *before* the caller's promise rejects, and `finally` then waits - bounded by `cancelGraceMs` (default 30s) - for the body to actually settle before running `env.cleanup()`. A body that observes the signal (which `runAgentInPostgresResearchEnvironment()`'s does) settles well inside that bound, so cleanup runs immediately behind the kill rather than racing it. A body that never looks at `signal` cannot hang cleanup forever either - past `cancelGraceMs`, cleanup proceeds regardless, and `PostgresCleanupResult.cancelGraceExceeded` records that this happened rather than hiding it.

In isolated mode, "the agent settles" specifically means `terminateResearchContainer()` (`server/postgres/agent-container.ts`) has been *awaited to completion* - not merely fired and left to run. It issues `docker kill` and does not resolve until docker confirms the outcome: success, "no such container" (already exited - every research container runs `--rm`, so this means already gone, which is the actual goal), or a genuine failure. Only once that has settled does `runAgentProcess()` escalate to SIGTERM/SIGKILL on the local `docker run` client process - so a client that later exits on its own is never mistaken for a container that was actually terminated by us. A failed `docker kill` is recorded on `PostgresResearchAgentResult.terminationError` rather than swallowed; cancellation still proceeds via the client-process escalation regardless, but that failure remains visible as a diagnostic.
...
- If the explicit `docker kill` in `terminateResearchContainer()` itself fails (a daemon hiccup, not merely "already gone"), the fallback is SIGTERM→SIGKILL on the local `docker run` client, which Docker's default `--sig-proxy` forwards into the container as SIGTERM. That forwarded SIGTERM is not immune to the container's own PID 1 being a shell blocked in a foreground command: POSIX shells defer handling a received signal - trapped or not - until the current foreground command returns control, so a container whose agent script is itself blocked in a long-running command can keep running for up to that command's remaining duration even after cancellation has been requested, despite `runAgentProcess()` itself resolving promptly (the *local client* dies, which is what it waits on). `docker kill`'s default signal is SIGKILL, which the kernel delivers unconditionally and is not subject to this - so this gap is specific to the *failure* fallback, not the supported success path, and does not affect any of `test/postgres-research-live-e2e.test.ts`'s non-failure-injected assertions. Hardening the fallback itself (e.g. retrying `terminateResearchContainer()`, or an escalating `docker kill --signal=KILL`) is future work, not attempted here (#188 review, second round).
