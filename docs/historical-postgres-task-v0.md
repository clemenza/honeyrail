...
- **Execution/attribution evidence** (grader-owned; explains *under what environment* the trial ran): `agent-result.json`, `workspace-inventory.json`, `agent-postgres.log` (the live PostgreSQL server log from the agent's own investigation session - copy failure is recorded as an `evidence_warning` diagnostic, never silently dropped).

  As of #209/PR #210, `agent-result.json` is an explicit, versioned (`schemaVersion: 1`) safe-session-evidence projection - **never** the raw session object. It carries `agent` (execution outcome: `ok`/`exitCode`/`signal`/`timedOut`/`timeoutSource`/`confirmedStopped`/`terminationError`/`startedAt`/`durationMs`), `isolation` (`mode`/`isolated`/`scoredEligible`/`networkMode`/`restrictedEgressVerified`/`imageIdentity`/`egressGateway` - the last present only for a real restricted-egress run, see #197), and `executionEnvironment` (the same already-public build/runtime identity described above). It never carries `agentEnvironment` (which includes any model API key passed to the agent), the source/build/runtime host paths, or grader-private truth. Any value present in the trial's `agent.env` long enough to plausibly be a credential is redacted (`[REDACTED]`) wherever it might otherwise appear - including inside `agent-stdout.txt`/`agent-stderr.txt`, `workspace-inventory.json`'s file paths, and the trajectory evidence below.

  `workspace-inventory.json` is a bounded-memory (`totalFiles`/`totalBytes` exact regardless of workspace size; `topLevelEntries`/`largestFiles` capped at 50 entries each, paths/counts only, never file contents) measurement of the agent's workspace, persisted regardless of whether the workspace turns out to be within the 2048-file/16MiB policy limit. If the agent's workspace exceeds that limit, the trial is `integrity_error` and the full `agent-workspace/` copy is skipped - but `agent-result.json`, `agent-stdout.txt`/`agent-stderr.txt`, `workspace-inventory.json`, and (when DSH engaged) `agent-transcript.ndjson` are still persisted (best-effort each; a failed write is recorded as an `evidence_warning` diagnostic rather than silently reclassifying the trial as `infrastructure_error`), so an over-limit trial is still inspectable rather than a dead end.

- **Agent process output** (grader-owned, captured by the driver, not written by the agent into its workspace): `agent-stdout.txt`, `agent-stderr.txt`. **`dsh --profile headless` prints its human-readable output only once, at normal process completion** - a trial killed at the agent timeout can leave both of these empty even though the agent did substantial work. They are not a complete trajectory source for a timed-out run; use the trajectory evidence below instead.

- **Agent trajectory evidence** (grader-owned, derived from DSH's own durable session log - #209/PR #210 round 4): `agent-transcript.ndjson`, `agent-session-stats.json`, `agent-trajectory.jsonl`.

  DSH's `@deepseek-ai/dsh-session-persistence-jsonl` plugin incrementally persists every raw session event to `$DSH_HOME/sessions/**` *as the trial runs*, independent of stdout. The Historical PostgreSQL agent container mounts a dedicated, per-trial, grader-owned host directory (`<pilot artifact dir>/dsh-home`) at `$DSH_HOME` for exactly this reason - deliberately separate from the agent's own workspace (`$HR_PG_WORK_DIR`), so this telemetry is never counted against the 2048-file/16MiB workspace policy and never mistaken for agent-authored task output. Because the mount is a host bind (not the container's `--rm`-destroyed ephemeral filesystem), whatever DSH wrote before a kill/timeout is already durable on disk by the time the driver reads it.

  - `agent-transcript.ndjson`: every raw session event, one JSON line per event, in order - the most important trajectory artifact, and non-empty even for a trial killed mid-run. **Required core evidence** for an otherwise scored-eligible, `agent.ok`, within-limit trial *when DSH actually engaged* (i.e. `$DSH_HOME/sessions/` exists at all) - such a trial is never graded, and never becomes an official dataset sample, if this could not be persisted. When DSH never engaged (a non-DSH agent, e.g. #180's own deterministic stub-agent smoke path) this artifact is simply absent and is not required - `runHistoricalPostgresTrial()` has no notion of "this profile is supposed to be a real DSH agent" at its own layer.
