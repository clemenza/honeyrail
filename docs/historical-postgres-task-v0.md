...
case/
  ...
  reference/                 # grader-owned only, never mounted into an agent
    ...
    expected-behavior/
      fix-evidence            # retained only when truth.knownFixEvidencePath was supplied
    verification/
...
**Case 001 backward compatibility (Policy A).** `truth.behavioralOracle` is a brand-new, optional field: for a spec that doesn't declare one (case 001, and any synthetic/unit-test spec), the `behavioralOracle` key is omitted from the serialized truth bundle entirely - not written as `null` - so case 001's serialized bytes, and therefore its hashes, are unaffected by this field's existence. (An earlier round of this work added it unconditionally as `null`, which - while `commitFest`'s `null`-when-absent handling was already correct, since case 001 always supplies a real `commitFest` value either way - did add a new key nothing previously had; that has been corrected.) `test/historical-postgres-002-task.test.ts`'s `"postgres-historical-001 still materializes and hashes under the widened truth schema"` test proves the key is genuinely absent, not merely null.

`truth.knownFixEvidencePath` is another brand-new optional field: a private host path (never committed, same discipline as `knownReproducerPath`) to grader-private fix/reference evidence - notes on the upstream fix, a diff, release-note excerpts, whatever an operator has. When supplied, it's copied byte-identical to `reference/expected-behavior/fix-evidence` (a fixed name, regardless of the source file's own name) and hashed (`fixEvidence`/`fixEvidenceSha256` in the truth bundle, covered by `expectedBehaviorSha256`/`bundleHash`). It is never read by the grader during scoring - purely provenance - and, unlike `behavioralOracle`, is recorded as `null` when absent (there are no pre-existing bytes to preserve for a field that never existed before). Neither `historicalPostgres001TaskSpec()` nor `historicalPostgres002TaskSpec()` populates it: doing so would require real upstream fix-diff content neither has been verified/fetched yet, and fabricating placeholder content would be worse than leaving the mechanism available-but-unused.
...
When a task declares one, `resolveOracleReproduction()` first calls `classifyExecutionValidity()` - a small, structural check of `execution.exitCode`/`execution.stderr` against known client/transport/runtime failure signatures (timeout, connection failure, container/docker transport errors, a host-side file-read failure), deliberately **not** based on how many observations were captured. Only when execution is valid does it extract the ordered sequence of psql `ERROR` message bodies from the submitted reproducer's own captured stderr (`extractPsqlErrorObservations()` - anchored to the real psql/libpq message-record shape, `LEVEL:  message` with the canonical two-space formatting, so `WARNING`/`NOTICE`/`DETAIL`/`HINT`/`CONTEXT`/`STATEMENT`/`LOG`/`INFO`/`DEBUG*` records and any arbitrary text merely *containing* the substring "ERROR:" are never mistaken for a real error record; this also strips any leading `psql:<file>:<line>:` label, so source path/line never enters an observation and never needs separate normalization) and calls `evaluateOracleAttribution()`, which matches those observations against **both** halves of the declared oracle and returns a structural `HistoricalPostgresOracleAttribution`:
