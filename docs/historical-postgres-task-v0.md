...
**Evaluation-partition note for case 003.** Because the real answer material (upstream bug id, both pinned revisions, and the expected oracle tuples) was previously committed to the public PR history of this repository (PR #204) before the corrected operator-supplied private-truth path was established, this specific case should be treated as a **historical/frontier regression case, not a pristine HOLDOUT case**, for strict evaluation-partition purposes. The framework's ability to materialize and grade a real third bug via the structured-output oracle is still valid evidence; the evaluation-partition status of the specific case is simply narrowed by its history. Future true-HOLDOUT cases must apply the same **pattern**: operator-supplied private truth (upstream bug id, both pinned revisions, and the expected oracle) is loaded at runtime from a local file that is never committed, validated strictly on load (not assumed correct), and never hardcoded in the repository. The current case-003 loader (`loadHistoricalPostgres003PrivateTruth()`) and its env-var convention (`HONEYRAIL_PG_199_*`) are case-specific; future cases should define their own analogous loader and env vars following the same discipline, not literally reuse these names.
...
#### Evidence: grader-owned execution/attribution, driver-captured process output, sanitized agent-trajectory diagnostics, and agent-owned

As of PR #210 review round 5, this evidence contract is split into four categories with materially different trust levels - not two ("agent-owned vs. grader-owned"). The middle two used to be described together as "grader-owned"; they are not equally trustworthy, and the distinction matters for anyone consuming this evidence.

- **Grader-owned execution/attribution evidence** (produced entirely by HoneyRail's own code, outside the agent's reach; explains *under what environment* the trial ran): `agent-result.json`, `workspace-inventory.json`, `agent-postgres.log` (the live PostgreSQL server log from the agent's own investigation session - copy failure is recorded as an `evidence_warning` diagnostic, never silently dropped), `grader/grade.json`.

  As of #209/PR #210, `agent-result.json` is an explicit, versioned (`schemaVersion: 1`) safe-session-evidence projection - **never** the raw session object. It carries `agent` (execution outcome: `ok`/`exitCode`/`signal`/`timedOut`/`timeoutSource`/`confirmedStopped`/`terminationError`/`startedAt`/`durationMs`), `isolation` (`mode`/`isolated`/`scoredEligible`/`networkMode`/`restrictedEgressVerified`/`imageIdentity`/`egressGateway` - the last present only for a real restricted-egress run, see #197), and `executionEnvironment` (the same already-public build/runtime identity described above). It never carries `agentEnvironment` (which includes any model API key passed to the agent), the source/build/runtime host paths, or grader-private truth. Any value present in the trial's `agent.env` long enough to plausibly be a credential is redacted (`[REDACTED]`) wherever it might otherwise appear - including inside `agent-stdout.txt`/`agent-stderr.txt`, `workspace-inventory.json`'s file paths, and the sanitized trajectory diagnostics below.

  `workspace-inventory.json` is a bounded-memory (`totalFiles`/`totalBytes` exact regardless of workspace size; `topLevelEntries`/`largestFiles` capped at 50 entries each, paths/counts only, never file contents) measurement of the agent's workspace, persisted regardless of whether the workspace turns out to be within the 2048-file/16MiB policy limit. If the agent's workspace exceeds that limit, the trial is `integrity_error` and the full `agent-workspace/` copy is skipped - but `agent-result.json`, `agent-stdout.txt`/`agent-stderr.txt`, `workspace-inventory.json`, and (when a DSH trajectory is expected) `agent-transcript.ndjson` are still attempted (best-effort each; a failed write is recorded as an `evidence_warning` diagnostic rather than silently reclassifying the trial as `infrastructure_error`), so an over-limit trial is still inspectable rather than a dead end.

- **Driver-captured process output** (captured by the driver from the agent process's own stdio streams, not written by the agent into its workspace): `agent-stdout.txt`, `agent-stderr.txt`. **`dsh --profile headless` prints its human-readable output only once, at normal process completion** - a trial killed at the agent timeout can leave both of these empty even though the agent did substantial work. They are not a complete trajectory source for a timed-out run; use the sanitized trajectory diagnostics below instead.

- **Sanitized agent-trajectory diagnostics** (derived and redacted by HoneyRail from DSH's own raw telemetry - #209/PR #210 rounds 4-5): `agent-transcript.ndjson`, `agent-session-stats.json`, `agent-trajectory.jsonl`.

  DSH's `@deepseek-ai/dsh-session-persistence-jsonl` plugin incrementally persists every raw session event to `$DSH_HOME/sessions/**` *as the trial runs*, independent of stdout. The Historical PostgreSQL agent container mounts `$DSH_HOME` at a **private, per-trial temporary directory outside the published trial artifact tree** - never under `<pilot artifact dir>/`, and never listed in `trial.artifacts` - deliberately separate from the agent's own workspace (`$HR_PG_WORK_DIR`), so this telemetry is never counted against the 2048-file/16MiB workspace policy and never mistaken for agent-authored task output. Because the mount is a host bind (not the container's `--rm`-destroyed ephemeral filesystem), whatever DSH wrote before a kill/timeout is already durable on disk by the time the driver reads it. HoneyRail bounded-reads this directory, derives and redacts the artifacts below, persists only those into the trial artifact tree, and removes the raw directory in a `finally` block once extraction completes - regardless of which outcome the trial reaches.

  **The raw `$DSH_HOME` source itself is not grader-owned or immutable evidence.** It is written by a process running *inside* the agent container, which has arbitrary shell access - it can read, modify, delete, or fabricate files under `$DSH_HOME` exactly as freely as under any other path it can reach. Do not describe it as "the agent can only write telemetry here"; that is not a valid trust statement once shell access is granted. The trusted grader-owned evidence is the *sanitized derived artifacts* below, produced by HoneyRail's own code after the container has exited, plus the execution/attribution evidence above - never the raw session log itself. Because the raw source can carry tool arguments, tool output, or assistant text that happens to include a known secret value, it must never be published alongside the sanitized artifacts that deliberately redact it: publishing the raw source into the same tree as the redacted derivative would defeat the redaction.

  Ingestion is bounded end to end (PR #210 review round 6), not just pre-parse-measured: `readBoundedDshSessionTelemetry()` (`server/evals/dsh-session-stats.ts`) walks `sessions/` with a streaming `opendir()` iterator - never `readdir(..., {recursive: true})`, which would materialize an entire, potentially agent-controlled tree into memory before any limit is ever consulted - and enforces five independent, explicit Historical-PG-specific bounds (`MAX_HISTORICAL_POSTGRES_DSH_TELEMETRY_ENTRIES`/`_FILES`/`_BYTES`/`_DECODED_BYTES`/`_EVENTS` in `historical-task.ts`) as it goes: every directory entry visited, matching `.jsonl`/`.jsonl.zstd` files, their on-disk (compressed) bytes, their decoded/plaintext bytes (a small compressed file can still expand into a much larger buffer - bounded independently via `zlib.zstdDecompressSync`'s own `maxOutputLength`, which rejects a frame before fully materializing an oversized decode, never after), and the total recovered raw events. A `.jsonl`/`.jsonl.zstd`-named entry that is a symlink (not a regular file per `lstat().isFile()`) is skipped outright - never followed, never counted, its target never opened - so a small symlink cannot smuggle a large target past the byte bound. This is a harness/evidence-retention safeguard, deliberately separate from the unrelated 2048-file/16MiB agent-workspace policy, since a `$DSH_HOME` an agent can write into has no other bound on its growth. Telemetry that exceeds any of these bounds is never parsed; it is reported as an `evidence_warning` and, for an otherwise scored-eligible run, treated the same as missing/unusable trajectory evidence below.

  The required transcript and the two derived, best-effort artifacts (`agent-session-stats.json`, `agent-trajectory.jsonl`) are each their own evidence sub-boundary (PR #210 review round 6): once `agent-transcript.ndjson` has been successfully persisted and found non-empty, a later failure deriving or persisting either best-effort artifact is recorded as its own `evidence_warning` and can never retroactively invalidate the already-usable transcript - an otherwise scored, within-limit DSH run still grades normally even if, say, `agent-trajectory.jsonl` alone failed to write. Cleanup of the private raw telemetry root is unconditional, but a cleanup *failure* is itself observable rather than silently swallowed: it is appended to the trial's diagnostics as its own `evidence_warning` (never including the private host path itself), without ever changing the trial's own status - a `blocked`/`integrity_error`/`completed` verdict already established stays exactly what it was.

  - `agent-transcript.ndjson`: every raw session event, one JSON line per event, in order - the most important trajectory artifact, and non-empty even for a trial killed mid-run. **Required core evidence** for an otherwise scored-eligible, `agent.ok`, within-limit trial whenever a DSH trajectory is expected for that trial - a caller-declared expectation (`trajectoryExpectation: "dsh"`, set explicitly by the TrialSet real-agent path, never inferred from `agent.command`, filesystem contents, `$DSH_HOME` existence after execution, or a profile id/name). A trial with no such expectation (e.g. #180's own deterministic stub-agent smoke path) never requires this artifact at all, and absence there is legitimately `not_applicable`. **A present-but-empty transcript does not satisfy this contract**: when a DSH trajectory is expected, at least one usable raw event must actually have been recovered - a zero-event/zero-line transcript is missing/incomplete required evidence, not success. Such a trial is never graded, and never becomes an official dataset sample, if this evidence is missing or unusable. Extraction failure (a filesystem read failure, corrupt JSONL, invalid JSON, zstd decode failure, or unexpected telemetry shape) is contained within its own evidence boundary and reported as an `evidence_warning`; it never overwrites an already-established `blocked` or workspace `integrity_error` attribution with `infrastructure_error`.
  - `agent-session-stats.json`: turn/step counts and LLM/tool/first-token/decode wall times, folded from the same raw log (`server/evals/dsh-session-stats.ts`). Derived/best-effort - regenerable from the transcript, so a failure here is a diagnostic, not a classification change.
  - `agent-trajectory.jsonl`: `tool_call`/`shell_command` events derived from the same raw log (`server/evals/dsh-trajectory-bridge.ts`) - what tools were called, with what arguments, and (for the `bash` tool) the command/stdout. Also derived/best-effort. Never fabricates a shell exit code DSH does not reliably expose.

- **Agent-owned** (written by the agent itself into its writable workspace, then copied grader-side as `agent-workspace/` only when the workspace is within the policy limit): `finding.json`, any reproducer file. Not immutable or grader-trusted the way the evidence above is - the agent process itself could in principle have written anything into its own workspace.
