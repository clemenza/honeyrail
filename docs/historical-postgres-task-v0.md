...
- **Grader-owned** (written by HoneyRail code, not the agent): `task-manifest.json`, `reference/truth.json`, `agent-result.json`, `agent-stdout.txt`/`agent-stderr.txt` (the agent process's own captured stdio), `workspace-inventory.json`, `agent-postgres.log` (the live PostgreSQL server log from the agent's own investigation session - copy failure is recorded as an `evidence_warning` diagnostic, never silently dropped), and `grader/grade.json`.

  As of #209/PR #210, `agent-result.json` is an explicit, versioned (`schemaVersion: 1`) safe-session-evidence projection - **never** the raw session object. It carries `agent` (execution outcome: `ok`/`exitCode`/`signal`/`timedOut`/`timeoutSource`/`confirmedStopped`/`terminationError`/`startedAt`/`durationMs`), `isolation` (`mode`/`isolated`/`scoredEligible`/`networkMode`/`restrictedEgressVerified`/`imageIdentity`), and `executionEnvironment` (the same already-public build/runtime identity described above). It never carries `agentEnvironment` (which includes any model API key passed to the agent), the source/build/runtime host paths, or grader-private truth. Any value present in the trial's `agent.env` long enough to plausibly be a credential is redacted (`[REDACTED]`) wherever it might otherwise appear - including inside `agent-stdout.txt`/`agent-stderr.txt` and `workspace-inventory.json`'s file paths.

  `workspace-inventory.json` is a bounded (`totalFiles`/`totalBytes` exact; `topLevelEntries`/`largestFiles` capped at 50 entries each, paths/counts only, never file contents) measurement of the agent's workspace, persisted regardless of whether the workspace turns out to be within the 2048-file/16MiB policy limit. If the agent's workspace exceeds that limit, the trial is `integrity_error` and the full `agent-workspace/` copy is skipped - but `agent-result.json`, `agent-stdout.txt`/`agent-stderr.txt`, and `workspace-inventory.json` are still persisted (best-effort each; a failed write is recorded as an `evidence_warning` diagnostic rather than silently reclassifying the trial as `infrastructure_error`), so an over-limit trial is still inspectable rather than a dead end.
